...
def sim_distance_(prefs, person1, person2):
    si = {}
    for item in prefs[person1]:
        if item in prefs[person2]:
            si[item] = 1
    ...
    for item in prefs[person1]:
        if item in prefs[person2]:
            product.append(pow(prefs[person1][item] - prefs[person2][item], 2))
    product_ = [pow(prefs[person1][item] - prefs[person2][item], 2) for item in prefs[person1] if
                item in prefs[person2]]
    return 1 / (1 + sqrt(sum(product_)))
...
def sim_pearson(prefs, p1, p2):
    ...
    if len(si) == 0:
        return 1
    # return 1 if len(si)==0 else None
...
if __name__ == '__main__':
    ...
    # print([i for i in critics['Gene Seymour']])