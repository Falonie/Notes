...
url='http://www.maigoo.com/maigoocms/special/ztjiaju/121KLY.html'
r=session.get(url=url,headers=headers,cookies=cookies).text
s=html.fromstring(r)
company=s.xpath('//ul[@class="info fr"]/li[@class="name"]/text()|'
                '//ul[@class="info long"]/li[@class="name"]/text()|'
                '//div[@class="b_company_info"]/div[@class="b-ttl"]/text()')
company=re.sub(r'[\n ]','',''.join(str(i).strip() for i in company))
print(company)

url='http://10.maigoo.com/search/?catid=2&areaid=3142&action=ajax&getac=brand&page=18'
